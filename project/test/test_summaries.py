...
def test_read_summary_incorrect_id(test_app_with_db: FastAPI):
    response = test_app_with_db.get("/summaries/531431/")
...
def test_update_summary_invalid_json(test_app_with_db: FastAPI):
    response = test_app_with_db.post(
        "/summaries/", data=json.dumps({"url": "https://foo.bar"})
    )
    summary_id = response.json()["id"]
...
def test_update_summary_invalid_keys(test_app_with_db: FastAPI):
    response = test_app_with_db.post(
        "/summaries/", data=json.dumps({"url": "https://foo.bar"})
    )
    summary_id = response.json()["id"]
